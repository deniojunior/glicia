"""Tipos compartilhados pelo domínio Python da Glicia."""
...
def merge_food_memory(
    current: Mapping[str, str], updates: tuple[FoodMemoryUpdate, ...]
) -> dict[str, str]:
    """Aplica somente preferências alimentares explícitas e não vazias."""
    merged = dict(current)
    for update in updates:
        food = update.food.strip().casefold()
        preparation = update.usual_preparation.strip()
        if food and preparation:
