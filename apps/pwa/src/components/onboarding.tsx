...
interface OnboardingProps {
  ...
  onComplete(preferences: PersistedPreferences): Promise<void>;
}
...
export function Onboarding({ initialProgress, onProgress, onComplete }: OnboardingProps) {
  ...
  async function complete(event: FormEvent<HTMLFormElement>) {
    ...
    setIsSaving(true);
    try {
      await onComplete({ version: 1, clinical_settings: progress.clinical_settings, interaction_mode: progress.interaction_mode });
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível salvar a configuração."); }
    finally { setIsSaving(false); }
  }

  return <main className="onboarding-shell">
    <header className="app-header"><span className="brand"><img src={gliciaIconUrl} width="44" height="44" alt="" /><span>Glicia</span></span><span className="setup-progress">Configuração {stepNumber(progress.step)} de 3</span></header>
    <section className="setup-content" aria-labelledby="setup-title">
      {progress.step === "welcome" ? <><h1 id="setup-title">Vamos deixar a Glicia pronta para você.</h1><p>Você vai informar os parâmetros já definidos com sua equipe de saúde. Leva poucos minutos.</p><InstallGlicia /><button className="primary-action" type="button" onClick={() => move("carbohydrate_ratios")}>Começar configuração</button></> : null}
...
function stepNumber(step: OnboardingProgress["step"]): number { return ["welcome", "carbohydrate_ratios", "review"].indexOf(step) + 1; }
