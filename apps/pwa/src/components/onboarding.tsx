...
export function Onboarding({ initialProgress, onProgress, onComplete }: OnboardingProps) {
  ...
  return <main className="onboarding-shell">
    ...
    <section className="setup-content" aria-labelledby="setup-title">
      {progress.step === "welcome" ? <><h1 id="setup-title">Vamos deixar a Glicia pronta para você.</h1><p>Você vai informar sua chave de IA e os parâmetros já definidos com sua equipe de saúde. Leva poucos minutos.</p><button className="primary-action" type="button" onClick={() => move("provider")}>Começar configuração</button></> : null}
      {progress.step === "provider" ? <form onSubmit={(event) => { event.preventDefault(); if (!apiKey.trim()) { setError("Cole sua chave da OpenAI para continuar."); return; } move("carbohydrate_ratios"); }}><h1 id="setup-title">Sua chave, nesta sessão.</h1><p>Ela não será salva no aparelho. Crie uma em platform.openai.com/api-keys.</p><label htmlFor="api-key">Chave da OpenAI</label><input id="api-key" type="password" autoComplete="off" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder="sk-..." /><div className="setup-actions"><button className="secondary-action" type="button" onClick={() => move("welcome")}>Voltar</button><button className="primary-action" type="submit">Continuar</button></div></form> : null}
