...
function OnboardingMessage({ step }: { step: OnboardingProgress["step"] }) {
  ...
  if (step === "safety") return <><p>Depois da sua confirmação, eu faço o cálculo com os dados do seu tratamento — glicemia, relação insulina-carboidrato, fator de correção e demais parâmetros — e mostro uma sugestão de dose.</p><p>É rápido, simples e seguro: a Glicia não lê seu sensor, não aplica insulina e não substitui sua equipe de saúde.</p></>;
  ...
  if (step === "carbohydrate_ratios") return <><p><strong>RIC significa relação insulina-carboidrato.</strong> É a quantidade de carboidrato, em gramas, coberta por uma unidade de insulina.</p><p>Agora me conte o valor definido pela sua equipe de saúde para cada refeição.</p></>;
...
function RatiosForm({ settings, onChange, onBack, onNext }: { settings: ClinicalSettings; onChange(key: (typeof mealFields)[number][0], raw: string): void; onBack(): void; onNext(): void }) {
  return <form onSubmit={(event) => { event.preventDefault(); onNext(); }} className="onboarding-form"><div className="settings-fields">{mealFields.map(([key, label]) => <NumberField key={key} label={`${label} — relação insulina-carboidrato`} unit="g por 1 U" value={settings.carbohydrate_ratios[key]} onChange={(value) => onChange(key, value)} />)}</div><StepActions back={onBack} next={() => undefined} nextLabel="Continuar" submit /></form>;
}

function Review({ settings }: { settings: ClinicalSettings }) { return <dl className="review-list"><div><dt>Glicemia-alvo</dt><dd>{settings.target_glucose} mg/dL</dd></div><div><dt>Fator de correção</dt><dd>{settings.correction_factor} mg/dL por U</dd></div><div><dt>Limite de hipoglicemia</dt><dd>{settings.hypoglycemia_threshold} mg/dL</dd></div>{mealFields.map(([key, label]) => <div key={key}><dt>RIC — {label.toLocaleLowerCase("pt-BR")}</dt><dd>{settings.carbohydrate_ratios[key]} g por 1 U</dd></div>)}</dl>; }
...
function titleFor(step: OnboardingProgress["step"]): string { const titles: Record<OnboardingProgress["step"], string> = { welcome: "Que bom ter você aqui.", how_it_works: "Eu busco os dados para você.", safety: "Você confirma cada passo.", clinical_settings: "Vamos conferir seus parâmetros?", carbohydrate_ratios: "O que significa RIC?", interaction_mode: "Tudo pronto para começar.", review: "Tudo pronto para começar." }; return titles[step]; }
