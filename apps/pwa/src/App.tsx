  ...
  return (
    <main className="app-shell">
      ...
      <section className="conversation" id="conversation" aria-labelledby="conversation-title">
        ...
        {isAwaitingConfirmation && currentTurn ? <aside className="confirmation-card" aria-labelledby="confirmation-title"><p className="eyebrow">Confira antes de decidir</p><h2 id="confirmation-title">Resumo da refeição</h2>{currentTurn.meal_items.length > 0 ? <ul className="confirmation-items">{currentTurn.meal_items.map((item, index) => <li key={`${item.name}-${index}`}><span>{item.name} · {item.portion}</span><strong>{item.carbohydrates} g</strong></li>)}</ul> : null}<dl><div><dt>Carboidratos</dt><dd>{currentTurn.total_carbohydrates} g</dd></div><div><dt>Glicemia</dt><dd>{currentTurn.glucose} mg/dL</dd></div><div><dt>Tendência</dt><dd>{currentTurn.glucose_trend?.replaceAll("_", " ").toLocaleLowerCase("pt-BR")}</dd></div><div><dt>Refeição</dt><dd>{mealTypeLabel(currentTurn.meal_type)}</dd></div></dl><button className="primary-action" type="button" onClick={confirm}>Confirmar dados</button><p className="card-note">O cálculo local só acontece depois da sua confirmação.</p></aside> : null}
