...
export class IndexedDbMealRepository implements MealHistoryRepository, FoodMemoryRepository {
  ...
  private async delete(key: string): Promise<void> { const db = await this.open(); await request(db.transaction(STORE_NAME, "readwrite").objectStore(STORE_NAME).delete(key)); db.close(); }
